    ...
    <div class="content-wrapper bg-white">
        <div class="border border-blue-100 mb-3 rounded-lg p-2">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('brand.index') }}" class="bg-blue-400 text-white px-4 py-2 rounded-lg">
                    Quay lại danh sách
                </a>
            </div>
        </div>
                    ...
                    @forelse ($list as $item)
                        <tr>
                            <td class="border border-gray-300 p-2 text-center">
                                @if($item->image)
                                    <img src="{{ asset($item->image) }}" class="w-16 h-16 object-cover rounded-md mx-auto" alt="Hình ảnh">
                                @else
                            ...
                            <td class="border border-gray-300 p-2 text-center space-y-2">
                                <!-- Restore -->
                                <a href="{{ route('brand.restore', ['brand' => $item->id]) }}" class="inline-block text-green-600 hover:underline">
